  ...
  return (
    <div className="flex flex-col h-full px-4 pt-4 overflow-y-auto scrollbar-hide">
      ...
      <div className="glass-card p-2 mb-4">
        ...
        <button onClick={addFilesFromPC} disabled={isScanning} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5">
          <FilePlus size={18} className="text-primary" />
          <div className="text-left">
            <p className="text-sm font-medium">Add Audio Files</p>
            <p className="text-xs text-muted-foreground">Select individual files</p>
          </div>
        </button>

        <button onClick={addFolderFromPC} disabled={isScanning} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5">
          <FolderOpen size={18} className="text-primary" />
          <div className="text-left">
            <p className="text-sm font-medium">Add Music Folder</p>
            <p className="text-xs text-muted-foreground">Scan an entire folder</p>
          </div>
        </button>

        {isNative && (
          <button onClick={rescan} disabled={isScanning} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5">
            {isScanning ? <Loader2 className="animate-spin text-primary" /> : <FolderSearch className="text-primary" />}
            <div>
              <p className="text-sm font-medium">{isScanning ? "Scanning..." : "Scan Device Storage"}</p>
            </div>
          </button>
        )}
      </div>
        ...
        <div className="flex items-center justify-between p-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Choose your app appearance</p>
            </div>
          </div>
      ...
      <div className="glass-card p-2 mb-4">
        ...
        <div className="flex items-center justify-between p-3">
          ...
          <div className="flex">
            {(["Auto", "High", "Standard"] as const).map((q) => (
              <button key={q} onClick={() => setQuality(q)} className={`px-2 py-1 ${quality === q ? "bg-primary text-white" : ""}`}>
                {q}
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setShowTimerPicker(!showTimerPicker)} className="p-3">
          Sleep Timer
        </button>
      </div>
      ...
      <div className="glass-card p-5">
        <p className="font-bold">Harmony Player</p>
